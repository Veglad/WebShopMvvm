package com.example.vshcheglov.webshop.presentation.helpers.custom_drawable
...
class CountDrawable(context: Context) : Drawable() {
    ...
    init {
        ...
        mBadgePaint = Paint()
        mBadgePaint.color = ContextCompat.getColor(context.applicationContext, R.color.icon_count_bg)
    ...
    override fun draw(canvas: Canvas) {
        ...
        if (mCount.length <= 2) {
            // Draw badge circle.
            canvas.drawCircle(centerX, centerY.toFloat(), (radius + 5.5f), mBadgePaint)
        } else {
            canvas.drawCircle(centerX, centerY.toFloat(), (radius + 6.5f), mBadgePaint)
        }
